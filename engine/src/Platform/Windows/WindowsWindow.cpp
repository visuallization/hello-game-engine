...
namespace Hello {
	...
	static void GLFWErrorCallback(int error, const char* description) {
		HO_CORE_ERROR("GLFW Error ({0}): {1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props) {
		...
		m_Data.Width = props.With;
		m_Data.Height = props.Height;

		HO_CORE_INFO("Creating window {0} ({1}, {2})", props.Title, props.With, props.Height);

		if (!s_GLFWInitialized) {
			// TODO: implement glfwTerminate on system shutdown
			int success = glfwInit();
			HO_CORE_ASSERT(success, "Could not initialize GLFW!");
			glfwSetErrorCallback(GLFWErrorCallback);
			s_GLFWInitialized = true;
		}

		// Set OpenGL context hints before creating window
